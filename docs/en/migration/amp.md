...
MMipt 1.x use `AmpOptimWrapper` provided by MMEngine.
...
To avoid user modifications to the configuration file, MMipt provides the `--amp` option in `train.py`, which allows the user to start AMP training without modifying the configuration file.
