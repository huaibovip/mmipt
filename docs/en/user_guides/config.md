# Tutorial 1: Learn about Configs in MMipt
...
- [Tutorial 1: Learn about Configs in MMipt](#tutorial-1-learn-about-configs-in-mmipt)
...
we make a brief comments on the [config of the EDSR model](https://github.com/huaibovip/mmipt/blob/main/configs/edsr/edsr_x2c64b16_g1_300k_div2k.py) we implemented as the following.
...
In MMipt's config, we use model fields to set up a model.
...
```python
default_scope = 'mmipt' # Used to set registries location
...
Taking [Stylegan2 at 1024x1024 scale](https://github.com/huaibovip/mmipt/blob/main/configs//styleganv2/stylegan2_c2_8xb4-fp16-global-800kiters_quicktest-ffhq-256x256.py) as an example,
...
Users can combine multiple schedulers to create a desired parameter adjustment strategy.
Find more in [parameter scheduler tutorial](https://mmengine.readthedocs.io/en/latest/tutorials/param_scheduler.html).
Since StyleGAN2 do not use parameter scheduler, we use config in [CycleGAN](https://github.com/huaibovip/mmipt/blob/main/configs/cyclegan/cyclegan_lsgan-id0-resnet-in_1xb1-250kiters_summer2winter.py) as an example:
...
```python
default_scope = 'mmipt'  # The default registry scope to find modules. Refer to https://mmengine.readthedocs.io/en/latest/advanced_tutorials/registry.html
