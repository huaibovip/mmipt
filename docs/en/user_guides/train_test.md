# Tutorial 4: Train and test in MMipt

In this section, we introduce how to test and train models in MMipt.

In this section, we provide the following guides:

- [Tutorial 4: Train and test in MMipt](#tutorial-4-train-and-test-in-mmipt)
  - [Prerequisite](#prerequisite)
  - [Test a model in MMipt](#test-a-model-in-mmipt)
  ...
  - [Train a model in MMipt](#train-a-model-in-mmipt)
...
Users need to [prepare dataset](../user_guides/dataset_prepare.md) first to enable training and testing models in MMipt.

## Test a model in MMipt
...
MMipt supports testing with multiple GPUs,
...
If you run MMipt on a cluster managed with [slurm](https://slurm.schedmd.com/), you can use the script `slurm_test.sh`. (This script also supports single machine testing.)
...
MMipt provides various **evaluation metrics**, i.e., MS-SSIM, SWD, IS, FID, Precision&Recall, PPL, Equivarience, TransFID, TransIS, etc.
We have provided unified evaluation scripts in [tools/test.py](https://github.com/huaibovip/mmipt/tree/main/tools/test.py) for all models.
...
If you are in slurm environment, please switch to the [tools/slurm_test.sh](https://github.com/huaibovip/mmipt/tree/main/tools/slurm_test.sh) by using the following commands:

```shell
sh slurm_test.sh ${PLATFORM} ${JOBNAME} ${CONFIG_FILE} ${CKPT_FILE}
```

## Train a model in MMipt

MMipt supports multiple ways of training:
...
If you run MMipt on a cluster managed with [slurm](https://slurm.schedmd.com/), you can use the script `slurm_train.sh`. (This script also supports single machine training.)
...
You can check [slurm_train.sh](https://github.com/huaibovip/mmipt/blob/master/tools/slurm_train.sh) for full arguments and environment variables.
