...
MMIPT_ROOT = Path(__file__).absolute().parents[3]
...
def update_model_zoo():
    ...
    model_index_file = MMIPT_ROOT / 'model-index.yml'
    ...
    assert len(collections) == len(os.listdir(MMIPT_ROOT / 'configs')) - 1

    configs = set([str(MMIPT_ROOT / m.config) for m in full_models])
    base_configs = glob(
        str(MMIPT_ROOT / 'configs/_base_/**/*.py'), recursive=True)
    all_configs = glob(str(MMIPT_ROOT / 'configs/**/*.py'), recursive=True)
    ...
    for task, models in tqdm(tasks.items(), desc='create markdown files'):
        ...
        for m in models:
            if m.readme not in readme:
                readme.add(m.readme)
                with open(MMIPT_ROOT / m.readme, 'r') as f:
                    c = f.read()
                content += c.replace('# ', '## ')
        overview = (f'# {task}\n\n'
                    f'## Summary\n'
