...
Welcome to the MMipt community, we are committed to building a Multimodal Advanced, Generative, and Intelligent Creation Toolbox.
...
```shell
git clone git@github.com:{username}/mmipt.git
```

After that, you should ddd official repository as the upstream repository

```bash
git remote add upstream git@github.com:open-mmlab/mmipt
```

Check whether remote repository has been added successfully by `git remote -v`

```bash
origin	git@github.com:{username}/mmipt.git (fetch)
origin	git@github.com:{username}/mmipt.git (push)
upstream	git@github.com:open-mmlab/mmipt (fetch)
upstream	git@github.com:open-mmlab/mmipt (push)
```
...
You should configure [pre-commit](https://pre-commit.com/#intro) in the local development environment to make sure the code style matches that of OpenMMLab. **Note**: The following code should be executed under the mmipt directory.
...
- mmipt introduces mypy to do static type checking to increase the robustness of the code. Therefore, we need to add Type Hints to our code and pass the mypy check. If you are not familiar with Type Hints, you can refer to [this tutorial](https://docs.python.org/3/library/typing.html).
...
mmipt will run unit test for the posted Pull Request on different platforms (Linux, Window, Mac), based on different versions of Python, PyTorch, CUDA to make sure the code is correct. We can see the specific test information by clicking `Details` in the above image so that we can modify the code.
