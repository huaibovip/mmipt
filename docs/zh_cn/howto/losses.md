...
`losses` 在 `MMipt` 中注册为 `LOSSES`。
在 MMipt 中设计自己的损失函数，步骤和在 MMipt 中自定义任何其他模型类似。
本节主要具体介绍了如何在 MMipt 中实现自定义的损失函数。
...
Class MSELoss(nn.Module)：
     ...
     def __init__(self, loss_weight=1.0, reduction='mean', sample_wise=False):
         # 代码可以在``mmipt/models/losses/pixelwise_loss.py``中找到
...
请注意，上面的`pixel_loss`必须在模型中定义。 详情请参考[自定义模型](./models.md)。 与自定义模型类似，为了使用您自己实现的损失函数，您需要在编写后在`mmipt/models/losses/__init__.py`中导入该损失函数。

### DiscShiftLoss 的一个例子

一般来说，要实现一个损失模块，我们会编写一个函数实现，然后用类实现包装它。
但是，在 MMipt 中，我们提供了另一个统一的接口 data_info 供用户定义输入参数和数据项之间的映射。
...
Class DiscShiftLoss(nn.Module)：
     ...
     def __init__(self, loss_weight=1.0, data_info=None):
         ...
         # 代码可以在``mmipt/models/losses/disc_auxiliary_loss.py``中找到
