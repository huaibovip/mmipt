...
欢迎加入 MMipt 社区，我们致力于打造新一代人工智能内容生成（AIGC）工具箱，我们欢迎任何类型的贡献，包括但不限于
...
在本地开发环境中，我们使用 [pre-commit](https://pre-commit.com/#intro) 来检查代码风格，以确保代码风格的统一。在提交代码，需要先安装 pre-commit（需要在 mmipt 目录下执行）:
...
- mmipt 引入了 mypy 来做静态类型检查，以增加代码的鲁棒性。因此我们在提交代码时，需要补充 Type Hints。具体规则可以参考[教程](https://zhuanlan.zhihu.com/p/519335398)。
...
mmipt 会在不同的平台（Linux、Window、Mac），基于不同版本的 Python、PyTorch、CUDA 对提交的代码进行单元测试，以保证代码的正确性，如果有任何一个没有通过，我们可点击上图中的 `Details` 来查看具体的测试信息，以便于我们修改代码。
...
6. 如果引入了其他三方库，或借鉴了三方库的代码，请确认他们的许可证和 mmipt 兼容，并在借鉴的代码上补充 `This code is inspired from http://`
...
```python
# Correct
from mmipt.cnn.bricks import (Conv2d, build_norm_layer, DropPath, MaxPool2d,
                             Linear)
   ...
   class CheckpointHook(Hook):
       ...
       Args:
           ...
           file_client_args (dict, optional): Arguments to instantiate a
               FileClient. See :class:`mmipt.fileio.FileClient` for details.
