...
from mmipt.utils import MMIPT_CACHE_DIR, download_from_url
...
def _load_inception_from_url(inception_url: str) -> nn.Module:
    ...
    print_log(f'Try to download Inception Model from {inception_url}...',
              'current')
    try:
        path = download_from_url(inception_url, dest_dir=MMIPT_CACHE_DIR)
...
def prepare_inception_feat(dataloader: DataLoader,
    ...
    if inception_pkl is None:
        ...
        inception_pkl = osp.join(MMIPT_CACHE_DIR, inception_pkl)
    else:
        args = dict()
...
def prepare_vgg_feat(dataloader: DataLoader,
    ...
    if vgg_pkl is None:
        ...
        vgg_pkl = osp.join(MMIPT_CACHE_DIR, vgg_pkl)
    else:
        args = dict()
