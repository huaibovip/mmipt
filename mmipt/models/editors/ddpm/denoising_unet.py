...
class DenoisingUnet(BaseModule):
    ...
    Args:
        img_size (int | list[int]): The size of image to denoise.
    ...
    def __init__(self,
        ...
        # check type of img_size
        if not isinstance(img_size, int) and not isinstance(
                img_size, list):
            raise TypeError(
                'Only support `int` and `list[int]` for `img_size`.')
        if isinstance(img_size, list):
            assert len(
                img_size) == 2, 'The length of `img_size` should be 2.'
        ...
        if isinstance(channels_cfg, dict):
            if img_size not in channels_cfg:
                raise KeyError(f'`img_size={img_size} is not found in '
                               '`channels_cfg`, only support configs for '
                               f'{[chn for chn in channels_cfg.keys()]}')
            self.channel_factor_list = channels_cfg[img_size]
        ...
        attention_scale = [img_size // int(res) for res in attention_res]
        ...
        if self.unet_type == 'stable':
            self.sample_size = img_size // 8  # NOTE: hard code here
