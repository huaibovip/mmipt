...
from mmengine.model.weight_init import (trunc_normal_, constant_init, normal_init,
                                        trunc_normal_init)
...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 1, 3, 5, 2, 4, 6,
                        7).contiguous().view(-1, window_size[0],
...
def window_reverse(windows, window_size, S, H, W):
    ...
    x = windows.view(B, S // window_size[0], H // window_size[1],
...
class PatchExpanding(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        ...
        if tag == 0:
            ...
            self.up = nn.ConvTranspose3d(
                dim, dim // 2, [2, 2, 2], [2, 2, 2], output_padding=[1, 0, 0])
    ...
    def forward(self, x, S, H, W):
        ...
        x = x.permute(0, 4, 1, 2, 3)
        x = self.up(x)
        x = x.permute(0, 2, 3, 4, 1).reshape(B, -1, C // 2)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        rel_coords[:, :, 2] += self.window_size[2] - 1

        rel_coords[:, :, 0] *= 3 * self.window_size[1] - 1
    ...
    def init_weights(self):
        trunc_normal_(self.rel_position_bias_table, std=0.02)
    ...
    def forward(self, x, mask=None, pos_embed=None):
        ...
        rel_position_bias = self.rel_position_bias_table[
            self.rel_position_index.view(-1)].view(
                self.window_size[0] * self.window_size[1] *
                self.window_size[2], self.window_size[0] *
                self.window_size[1] * self.window_size[2], -1)
        rel_position_bias = rel_position_bias.permute(2, 0, 1).contiguous()
        attn = attn + rel_position_bias.unsqueeze(0)
...
class WindowAttention_kv(nn.Module):
    ...
    def forward(self, skip, x_up, mask=None):
        ...
        kv = kv.reshape(B_, N, 2, self.num_heads,
                        C // self.num_heads).permute(2, 0, 3, 1,
                                                     4).contiguous()
        q = q.reshape(B_, N, self.num_heads,
...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        skip = F.pad(skip,
                     (0, 0, 0, pad_r, 0, pad_b, 0,
                      pad_g))  # pad�ĵ�һ�������Ӧ���һ��ά�ȣ����һ�������Ӧ��һ��ά��
        ...
        if downsample is not None:
            ...
            if i_layer == 1:
                self.downsample = downsample(
                    dim=dim, norm_layer=norm_layer, tag=1)
            elif i_layer == 2:
                self.downsample = downsample(
                    dim=dim, norm_layer=norm_layer, tag=2)
            elif i_layer == 0:
                self.downsample = downsample(
                    dim=dim, norm_layer=norm_layer, tag=0)
            else:
...
class project(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv3d(
            in_dim, out_dim, kernel_size=3, stride=stride, padding=padding)
    ...
    def forward(self, x):
        ...
        if not self.last:
            ...
            x = x.flatten(2).transpose(1, 2).contiguous()
            x = self.norm2(x)
            x = x.transpose(1, 2).contiguous().view(-1, self.out_dim, Ws, Wh,
                                                    Ww)
        return x
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if S % self.patch_size[0] != 0:
            x = F.pad(
                x,
                (0, 0, 0, 0, 0, self.patch_size[0] - S % self.patch_size[0]))
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).transpose(1, 2).contiguous()
            x = self.norm(x)
            x = x.transpose(1, 2).contiguous().view(-1, self.embed_dim, Ws, Wh,
                                                    Ww)
            ...
            for i in range(len(depths) - 1):
                self.final.append(
                    final_patch_expanding(
                        embed_dim * 2**i, num_classes, patch_size=patch_size))
        else:
            self.final.append(
                final_patch_expanding(
                    embed_dim, num_classes, patch_size=patch_size))
...
if __name__ == '__main__':
    ...
    data = torch.randn(1, 1, 64, 128, 128)
