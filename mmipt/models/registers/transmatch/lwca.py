...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        qkv2 = self.qkv(y).reshape(B_, N, 3, self.num_heads,
                                   C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, mask_matrix):
        ...
        y_windows = window_partition(
            shifted_y,
            self.window_size)  # nW*B, window_size, window_size, window_size, C
        y_windows = y_windows.view(
            -1,
            self.window_size[0] * self.window_size[1] * self.window_size[2],
            C)  # nW*B, window_size*window_size*window_size, C
        # W-MSA/SW-MSA
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                dim_diy=dim_diy) for i in range(depth)
        ])
        ...
        for i in range(1):  # num_layers  = len(depths)  = 4
            ...
            if i in self.out_indices:
                ...
                out = x_out.view(-1, H, W, T,
                                 self.num_features).permute(0, 4, 1, 2,
                                                            3).contiguous()
        return out
...
class LWCA(nn.Module):
    ...
    def forward(self, x, y):
        mov_feat_cross = self.transformer(x, y)
        return mov_feat_cross
