...
def window_partition(x, window_size):
    ...
    windows = (
        x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(
            -1,
            window_size[0],
            window_size[1],
            window_size[2],
...
def window_reverse(windows, window_size, H, W, D):
    ...
    B = int(windows.shape[0] /
            (H * W * D / window_size[0] / window_size[1] / window_size[2]))
    x = windows.view(
        B,
        H // window_size[0],
        W // window_size[1],
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            x = F.pad(x,
                      (0, 0, 0, self.patch_size[1] - W % self.patch_size[1]))
        if H % self.patch_size[0] != 0:
            x = F.pad(
                x,
                (0, 0, 0, 0, 0, self.patch_size[0] - H % self.patch_size[0]))
...
class SinPositionalEncoding3D(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.inv_freq = 1.0 / (10000**(torch.arange(0, channels, 2).float() /
                                       channels))
...
class TransMorph(nn.Module):
    ...
    def __init__(
        ...
        if self.if_convskip:
            ...
            self.c2 = Conv3dReLU(2, reg_head_chan, 3, 1, use_batchnorm=False)
...
@MODELS.register_module()
class TransMorphLarge(nn.Module):
