...
from . import finite_differences as fdt
...
def gen_identity_map(img_size, resize_factor=1.0, normalized=True):
    """given displacement field,  add displacement on grid field."""
    if isinstance(resize_factor, list):
        img_size = [int(img_size[i] * resize_factor[i]) for i in range(dim)]
    else:
        img_size = [int(img_size[i] * resize_factor) for i in range(dim)]
    if normalized:
        grid = (
            identity_map(img_size) if not reproduce_paper_result else
            identity_map_for_reproduce(img_size))
    else:
        grid = not_normalized_identity_map(img_size)
    return grid
...
def scale_map_grad(grad_map, spacing):
    ...
    for d in range(ndim):
        ...
        if sz[d + 2] > 1:
            grad_map[:, d, ...] *= 2.0 / (sz[d + 2] - 1.0) / spacing[d]
        else:
            grad_map[:, d, ...] = grad_map[:, d, ...]


class STNFunction_ND_BCXYZ(nn.Module):
    """Spatial transform function for 1D, 2D, and 3D.

    In BCXYZ format (this IS the format used in the current toolbox).
    """
    ...
    def forward(self, input1, input2):
        ...
        if self.using_01_input:
            output = self.forward_stn(input1, scale_map(input2, self.spacing),
                                      self.ndim)
        else:
...
class STN_ND_BCXYZ(nn.Module):
    """Spatial transform code for nD spatial transoforms.

    Uses the BCXYZ image format.
    """
...
class convBlock(nn.Module):
    """A convolutional block including conv, BN, nonliear activiation, residual
    connection."""
...
class TransMorphDiff(nn.Module):
    ...
    def __init__(
        ...
        self.low_res_img_size = [int(x / 2) for x in img_size]
        self.spacing = 1.0 / (np.array(img_size) - 1)
        self.int_steps = 7
        ...
        self.flow_vol_shape = self.low_res_img_size
        if not downsample:
            self.flow_vol_shape = self.img_size

        self.D = self._degree_matrix(self.flow_vol_shape).cuda()

        self.id_transform = gen_identity_map(self.img_size, 1.0).cuda()
        self.id_transform = self.id_transform.view(
            [1] + list(self.id_transform.shape))

        if not downsample:
            self.reg_res_id_transform = gen_identity_map(
                self.img_size, 1.0, normalized=False).cuda()
        """to compatiable to the mesh setting in voxel morph"""
        self.low_res_id_transform = gen_identity_map(
            self.img_size, 0.5, normalized=False).cuda()

        self.bilinear = STN_ND_BCXYZ(
            np.array([1.0, 1.0, 1.0]), zero_boundary=True)
        ...
        if not self.downsample:
            ...
            self.up4 = TM.DecoderBlock(
                embed_dim // 2,
                reg_head_chan,
                skip_channels=reg_head_chan if if_convskip else 0,
                use_batchnorm=False,
            )
            self.c2 = TM.Conv3dReLU(
                2, reg_head_chan, 3, 1, use_batchnorm=False)
        self.c1 = TM.Conv3dReLU(2, embed_dim // 2, 3, 1, use_batchnorm=False)

        self.flow_mean = nn.Conv3d(
            reg_head_chan, 3, kernel_size=3, stride=1, padding=1, bias=True)
        self.flow_sigma = nn.Conv3d(
            reg_head_chan, 3, kernel_size=3, stride=1, padding=1, bias=True)
        self.flow_mean.weight.data.normal_(0.0, 1e-5)
        self.flow_sigma.weight.data.normal_(0.0, 1e-10)
    ...
    def forward(self, source, target, **kwargs):
        ...
        for _ in range(self.int_steps):
            if not self.downsample:
                deform_field = flow + self.reg_res_id_transform
            else:
    ...
    def _adj_filt(self, ndims):
        ...
        so for each filter, the filter has 2^ndims 1s. the filter is then setup
        such that feature i outputs only to feature i
        """
        ...
        filt = np.zeros([ndims, ndims] +
                        [3] * ndims)  # 3 3 3 3  ##!!!!!!!! in out w h d
    ...
    def prec_loss(self, disp):  ##!!!!!!!!
        ...
        """
        fd = fdt.FD_torch(np.array([1.0, 1.0, 1.0]))
        dfx = fd.dXc(disp[:, 0, ...])
        dfy = fd.dYc(disp[:, 1, ...])
        dfz = fd.dZc(disp[:, 2, ...])
    ...
    def kl_loss(self):
        """KL loss y_pred is assumed to be D*2 channels: first D for mean, next
        D for logsigma D (number of dimensions) should be 1, 2 or 3 y_true is
        only used to get the shape."""
        # prepare inputs
        ndims = 3
        ...
        # sigma terms
        sigma_term = (self.prior_lambda * self.D * torch.exp(log_sigma) -
                      log_sigma)  ##!!!!!!!!
        sigma_term = torch.mean(sigma_term)  ##!!!!!!!!

        # precision terms
        prec_term = self.prior_lambda_mean * self.prec_loss(
            flow_mean)  # this is the jacobi loss
    ...
    def recon_loss(self):
        ...
        y_pred = self.warped
        y_true = self.target
        return (1.0 / (self.image_sigma**2) * torch.mean(
            (y_true - y_pred)**2))  ##!!!!!!!!
