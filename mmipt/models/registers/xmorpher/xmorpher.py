...
class CrossWindowAttention3D(nn.Module):
    ...
    def forward(self, x, xa):
        ...
        q = (
            self.q(x).reshape(B_, N, self.num_heads,
                              C // self.num_heads).permute(0, 2, 1, 3))

        kv = (
            self.kv(xa).reshape(B_, M, 2, self.num_heads,
...
class CrossTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, xa):
        ...
        attn_windows = self.cross_attn(x_windows,
                                       x_area_windows)  # B*nW, Wd*Wh*Ww, C
        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size + (C, )))
        x = window_reverse(attn_windows, window_size, B, Dp, Hp,
                           Wp)  # B D' H' W' C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        self.down_conv = nn.Conv3d(
            dim, 2 * dim, (2, 2, 2), stride=2, padding=0)
...
class PatchExpand(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        self.up_conv = nn.ConvTranspose3d(
            dim, dim // 2, (2, 2, 2), stride=2, padding=0)
