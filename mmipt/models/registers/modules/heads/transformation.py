...
class _Transform:
    ...
    def __call__(self, x):
        ...
        if self.svf:
            disp = svf_exp(flow, scale=self.svf_scale, steps=self.svf_steps)
            return flow, disp
        else:
            disp = flow
            return disp
...
class DenseTransform(_Transform):
    ...
    def __init__(self, svf=False, svf_steps=7, svf_scale=1):
        super().__init__(svf=svf, svf_steps=svf_steps, svf_scale=svf_scale)
...
class CubicBSplineFFDTransform(_Transform):
    ...
    def compute_flow(self, x):
        ...
        flow = x
        for i, (k, s,
                p) in enumerate(zip(self.kernels, self.stride, self.padding)):
...
def svf_exp(flow, scale=1, steps=5, sampling='bilinear'):
    ...
    for _ in range(steps):
        ...
        disp = disp + warp(disp=disp, image=disp, interp_mode=sampling)
    return disp
...
def conv1d(data: Tensor,
    ...
    weight = kernel.expand(groups, 1,
                           kernel.shape[-1])  # 3*w*d, 1, kernel_size
    result = result.reshape(shape_[0], groups,
                            shape_[-1])  # n, 3*w*d, shape[dim]
...
def warp(disp, image, interp_mode='bilinear'):
    """
