...
class AffineRegistrationHead(BaseRegistrationHead):
    """Head for affine registration.
    ...
    def forward_train(
        ...
        if self.with_seg_loss:
            source_seg = data_samples.get('source_seg')
            target_seg = data_samples.get('target_seg')
            num_classes = data_samples.get('num_classes', 30)
            source_seg_oh = F.one_hot(
                source_seg.long(), num_classes=num_classes)
            target_seg_oh = F.one_hot(
                target_seg.long(), num_classes=num_classes)

            # warp one-hot label with displacement field
            warped_seg_oh = self.warp(
                flow, source_seg_oh.float(), interp_mode='nearest')

            return self.losses(flow, target_img, warped_img, target_seg_oh,
                               warped_seg_oh)
    ...
    def forward_test(
        ...
        # warped_seg = warp(flow, source_seg.float(), interp_mode='nearest')
        warped_seg = self.warp(flow, source_seg.float(), interp_mode='nearest')
        data_samples.set_tensor_data(dict(fwd_pred_seg=warped_seg))
        data_samples.set_tensor_data(dict(fwd_grid=flow))
        data_samples.set_tensor_data(dict(fwd_flow=flow))

        # warp grid with displacement field
        if return_grid:
            warped_grid = self.get_warped_grid(flow)
            data_samples.set_tensor_data(dict(warped_grid=warped_grid))
