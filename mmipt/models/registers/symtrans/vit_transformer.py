...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # (b:n_samples, c:embed_dim, h:n_patches**0.5, w:n_patches**0.5)
        x = self.proj(x)
        # (b:n_samples, l:n_patches, c:embed_dim)
...
class DSAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, self.img_size[0],
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(
            x)  # (n_samples, n_patches+1,3*dim) get query,key,value.
        qkv = qkv.reshape(
            n_samples, n_tokens, 3, self.n_heads,
            self.head_dim)  # (n_samples,n_patches+1,qkv,n_heads,head_dim)
        ...
        attn = dp.softmax(
            dim=-1)  # (n_samples,n_heads,n_patches+1,n_patches+1)
        attn = self.attn_drop(attn)
        ...
        weighted_avg = rearrange(
            weighted_avg,
            'n_samples n_patches n_heads head_dim -> n_samples n_patches (n_heads head_dim)'
...
class PatchExpanding(nn.Module):
    ...
    def __init__(self, dim, input_shape, scale_factor=8, bias=False):
        ...
        self.expander = nn.Sequential(
            ...
            Rearrange(
                'b D H W (h d w c) -> b (D d) (H h) (W w) c',
                d=2,
                h=2,
                w=2,
                c=self.dim), Rearrange('b D H W c -> b (D H W) c'),
...
class VectorDownSample(nn.Module):
    ...
    def __init__(self, shape, dim, image_dim=2):
        ...
        self.downsample = nn.Sequential(
            nn.Linear(self.dim, self.dim * 2), Rearrange('B L C -> B C L'),
            nn.Linear(self.size, self.size // (2**image_dim)),
            Rearrange('B C L -> B L C'), nn.LayerNorm(self.dim * 2, eps=1e-6))
...
class SkipConnection_v2(nn.Module):
    ...
    def __init__(self, dim, input_shape, bias=False):
        ...
        self.fusion = nn.Conv3d(
            2 * dim, dim, kernel_size=3, stride=1, padding=1)
