...
def _expand_onehot_labels_dice(pred: torch.Tensor,
    ...
    one_hot_target = nn.functional.one_hot(one_hot_target, num_classes + 1)
...
def dice_loss(pred: torch.Tensor,
              ...
              ignore_index: Union[int, None] = -1) -> float:
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        union = (
            torch.pow(pred, 2).sum(dim=[2, 3, 4]) +
            torch.pow(true, 2).sum(dim=[2, 3, 4]))
        dsc = (2. * intersection) / (union + 1e-5)
        dsc = (1 - torch.mean(dsc)) * self.loss_weight
        return dsc
