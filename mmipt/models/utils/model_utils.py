...
from mmipt.utils.typing import ForwardInputs
from .tome_utils import (add_tome_cfg_hook, build_mmipt_tomesd_block,
                         build_mmipt_wrapper_tomesd_block, isinstance_str)
...
def set_tomesd(model: torch.nn.Module,
    ...
    is_mmipt = isinstance_str(model, 'StableDiffusion') or isinstance_str(
        model, 'BaseModel')
        ...
        if isinstance_str(module, 'BasicTransformerBlock'):
            ...
            if is_mmipt:
                if is_wrapper is None:
                    ...
                    make_tome_block_fn = build_mmipt_tomesd_block
                elif is_wrapper:
                    make_tome_block_fn = build_mmipt_wrapper_tomesd_block
            else:
                raise TypeError(
...
def remove_tomesd(model: torch.nn.Module):
    ...
    """
    # For mmipt Stable Diffusion models
